...
`,
  ...
  },
  {
    ...
    initialCode: `# ========== 项目2：购物车分析（关联规则Apriori） ==========
import pandas as pd
import numpy as np
...
`,
  ...
  },
  {
    ...
    initialCode: `# ========== 项目4：用户行为路径聚类（KMeans） ==========
import pandas as pd
import numpy as np
from sklearn.preprocessing import StandardScaler
from sklearn.cluster import KMeans
...
`,
  ...
  },
  {
    ...
    description: '异常检测、Z-score、孤立森林、多维特征。订单数据含金额、数量、折扣、用户注册时长等，使用Z-score与孤立森林标记异常订单。',
    difficulty: '进阶',
    skills: ['孤立森林', 'Z-score', '异常检测', '集成学习'],
...
amounts_normal = np.random.normal(normal_mean, normal_std, normal_count)
amounts_normal = np.clip(amounts_normal, 10, 300)

# 生成异常交易（金额极高或极低）
amounts_outlier_high = np.random.uniform(800, 5000, n_outliers // 2)
amounts_outlier_low = np.random.uniform(0, 5, n_outliers // 2)
amounts_outlier = np.concatenate([amounts_outlier_high, amounts_outlier_low])

amounts = np.concatenate([amounts_normal, amounts_outlier])
...
data = {
    ...
    'Discount': np.random.beta(1, 5, n_transactions) * 0.3,  # 折扣率
    'User_Age_Days': np.random.exponential(365, n_transactions).astype(int),  # 注册天数
    'Hour': np.random.randint(0, 24, n_transactions),
    'Weekend': np.random.choice([0, 1], n_transactions, p=[0.7, 0.3])
}

df = pd.DataFrame(data)

# 人为添加更多异常模式
df.loc[100:110, 'Quantity'] = 50  # 批量购买异常
df.loc[200:205, 'Discount'] = 0.95  # 极高折扣
df.loc[300:305, 'User_Age_Days'] = 0  # 新用户异常
...
# 对金额列应用Z-score
zscore_outliers = detect_outliers_zscore(df, 'Amount', threshold=3)
...
iqr_outliers = detect_outliers_iqr(df, 'Amount')
...
df['IF_Outlier'] = iso_forest.fit_predict(X_scaled)
df['IF_Outlier'] = df['IF_Outlier'] == -1  # -1表示异常

print(f"孤立森林检测到异常数：{df['IF_Outlier'].sum()}")

# 7. 集成方法（投票）
df['Vote_Count'] = (df['Zscore_Outlier'].astype(int) + 
...
print(f"\n异常交易占比：{len(outliers)/len(df)*100:.2f}%")

print("\n特征对比（异常 vs 正常）：")
for col in ['Amount', 'Quantity', 'Discount', 'User_Age_Days']:
    outlier_mean = outliers[col].mean()
    normal_mean = normal[col].mean()
    print(f"  {col}: 异常={outlier_mean:.2f}, 正常={normal_mean:.2f}, 倍数={outlier_mean/normal_mean:.2f}")

# 9. 验证异常检测的一致性
print("\n=== 方法一致性验证 ===")
method_overlap = pd.crosstab(df['Zscore_Outlier'], df['IF_Outlier'])
print("Z-score vs 孤立森林重合度：")
print(method_overlap)
...
jaccard_zscore_if = jaccard_similarity(df['Zscore_Outlier'], df['IF_Outlier'])
jaccard_zscore_iqr = jaccard_similarity(df['Zscore_Outlier'], df['IQR_Outlier'])
jaccard_iqr_if = jaccard_similarity(df['IQR_Outlier'], df['IF_Outlier'])

print(f"\nJaccard相似度：")
print(f"  Z-score vs IQR: {jaccard_zscore_iqr:.3f}")
print(f"  Z-score vs 孤立森林: {jaccard_zscore_if:.3f}")
print(f"  IQR vs 孤立森林: {jaccard_iqr_if:.3f}")
...
axes[0,0].set_title('交易金额分布（正常 vs 异常）')
axes[0,0].legend()

# 箱线图多特征对比
outlier_status = df['Ensemble_Outlier'].map({True: '异常', False: '正常'})
df_box = df[['Amount', 'Quantity', 'Discount']].copy()
df_box['Status'] = outlier_status

for i, col in enumerate(['Amount', 'Quantity', 'Discount']):
    axes[1,0].boxplot([df[df['Ensemble_Outlier']==False][col], 
                       df[df['Ensemble_Outlier']==True][col]], 
                      labels=['正常', '异常'], positions=[i*2, i*2+1], widths=0.6)
axes[1,0].set_xticks([0.5, 2.5, 4.5])
axes[1,0].set_xticklabels(['金额', '数量', '折扣'])
axes[1,0].set_ylabel('值')
axes[1,0].set_title('各特征异常对比')

# 方法重叠韦恩图（简化版）
methods_data = {
    'Z-score': set(df[df['Zscore_Outlier']].index),
    'IQR': set(df[df['IQR_Outlier']].index),
    '孤立森林': set(df[df['IF_Outlier']].index)
}

from matplotlib_venn import venn3
venn3([methods_data['Z-score'], methods_data['IQR'], methods_data['孤立森林']],
      set_labels=('Z-score', 'IQR', '孤立森林'), ax=axes[0,1])
axes[0,1].set_title('三种方法检测到的异常重叠')

# 异常得分分布
df['Anomaly_Score'] = df['Vote_Count'] / 3
sns.histplot(data=df, x='Anomaly_Score', hue='Ensemble_Outlier', bins=20, ax=axes[1,1])
axes[1,1].set_title('异常得分分布')
axes[1,1].set_xlabel('异常得分（0-1）')
...
print("\n异常类型分析：")
high_amount = outliers[outliers['Amount'] > 500]
high_qty = outliers[outliers['Quantity'] > 10]
high_discount = outliers[outliers['Discount'] > 0.5]
print(f"  高金额异常：{len(high_amount)}笔")
print(f"  高数量异常：{len(high_qty)}笔")
print(f"  高折扣异常：{len(high_discount)}笔")`
`,
    tips: ['Z-score适合单变量高斯分布的数据', '孤立森林擅长处理多维异常检测', '集成多种方法可以提高检测准确度']
  },
  {
    id: 'bi-project-10',
    chapterId: 'chapter-62',
    title: '端到端BI仪表盘（综合项目）',
    description: '前述所有+数据管道+仪表盘输出。给定多表数据，自行清洗合并，完成购物车分析+RFM+聚类+异常检测，动态筛选器展示KPI、图表、聚类客户画像。',
    difficulty: '综合',
    skills: ['数据整合', 'RFM', '聚类', '关联规则', 'BI仪表板'],
...
# 商品表
products = pd.DataFrame({
    'Product_ID': [f'P_{i:04d}' for i in range(80)],
...
# 3. 购物车分析（关联规则）
# 构建购物车矩阵
cart_data = df_sales.groupby(['Order_ID', 'Category']).size().unstack(fill_value=0)
cart_matrix = (cart_data > 0).astype(bool)
...
repeat_purchase_rate = len(rfm[rfm['Frequency'] > 1]) / total_users * 100

print(f"总销售额：¥{total_revenue:,.0f}")
print(f"总订单数：{total_orders}")
print(f"总用户数：{total_users}")
print(f"平均客单价：¥{avg_order_value:.0f}")
print(f"复购率：{repeat_purchase_rate:.1f}%")

# 7. 验证函数
def validate_data_quality():
    """验证数据质量"""
    checks = {
        '无重复订单': len(df_orders['Order_ID']) == len(df_orders['Order_ID'].unique()),
        '金额为正': (df_sales['Amount'] > 0).all(),
        '数量为正': (df_sales['Quantity'] > 0).all(),
        '用户存在': df_sales['User_ID'].isin(users['User_ID']).all(),
        '商品存在': df_sales['Product_ID'].isin(products['Product_ID']).all()
    }
...
fig.suptitle('电商BI仪表盘', fontsize=20, fontweight='bold')
...
ax2.set_title('各品类销售占比')

# 3) RFM客户分层
segment_counts = pd.cut(rfm['RFM_Total'], bins=[0, 5, 8, 11, 12], 
                        labels=['低价值', '中价值', '高价值', '超高价值']).value_counts()
ax3 = fig.add_subplot(2, 3, 3)
ax3.bar(segment_counts.index, segment_counts.values, color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4'])
ax3.set_title('客户价值分层')
ax3.set_xlabel('客户类型')
ax3.set_ylabel('客户数')
...
如需交互式仪表盘，建议使用以下工具：
- Streamlit：运行 'streamlit run dashboard.py'
- Plotly Dash：创建Web应用
- Jupyter Widgets：添加下拉菜单、日期选择器等
""")
...
verification = {
    '数据清洗': len(df_sales) > 0,
    '购物车分析': len(rules) > 0 if len(rules) > 0 else '无规则（数据稀疏）',
    'RFM分析': len(rfm) > 0,
    '用户聚类': 'Cluster' in user_features.columns if 'Cluster' in user_features.columns else False,
    '可视化': True
}
...
print("\n✅ 端到端BI仪表盘项目完成！")
print("该项目整合了数据清洗、购物车分析、RFM分层、用户聚类等核心数据分析技术。")`
`,
    tips: ['多表数据清洗和整合是BI分析的基础', '将多个分析模块整合到一个完整流程', '可视化的关键在于找到最有价值的洞察']
  }
import { useParams, useNavigate } from 'react-router-dom';
import { ArrowLeft, Code2, Lightbulb } from 'lucide-react';
import PythonRunner from '../components/PythonRunner';
